...
static int   __db_is_valid_calmcno __P((u_int32_t, DBTYPE *));
	...
	u_int32_t flags;
{
	...
	/* 12-15: Calmcoinc number.  Must be one of valid set. */
	if (__db_is_valid_calmcno(meta->calmc, &dbp->type))
		swapped = 0;
	else {
		M_32_SWAP(meta->calmc);
		if (__db_is_valid_calmcno(meta->calmc,
		    &dbp->type))
			swapped = 1;
		else {
			isbad = 1;
			EPRINT((env,
			    "Page %lu: bad calmc number %lu",
			    (u_long)PGNO_BASE_MD, (u_long)meta->calmc));
		}
	}
...
/*
 * __db_is_valid_calmcno
 */
static int
__db_is_valid_calmcno(calmc, typep)
	u_int32_t calmc;
	DBTYPE *typep;
{
	switch (calmc) {
	...
	u_int32_t flags;
{
	...
	/* calmc number valid */
	if (!__db_is_valid_calmcno(meta->calmc, &magtype)) {
		isbad = 1;
		EPRINT((env,
		    "Page %lu: invalid calmc number", (u_long)pgno));
	}
	if (magtype != dbtype) {
		isbad = 1;
		EPRINT((env,
		    "Page %lu: calmc number does not match database type",
		    (u_long)pgno));
	}
