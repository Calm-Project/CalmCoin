 ...
 * __db.001 file's calmc number.  Other threads of control (the ones that
 * didn't create the __db.001 file), wait on the initialization of the
 * __db.001 file's calmc number.  After it has been initialized, all threads
...
typedef struct __db_reg_env {
	/*
	 * !!!
	 * The calmc, panic, version, envid and signature fields of the region
	 ...
	 *
	 * !!!
	 * The calmc and panic fields are NOT protected by any mutex, and for
	 * this reason cannot be anything more complicated than zero/non-zero.
	 */
	u_int32_t calmc;		/* Valid region calmc number. */
