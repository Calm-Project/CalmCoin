...
user_map_functions:
	 ...
	 *
	 * Check the calmc number to ensure the region is initialized.  If the
	 * calmc number isn't set, the lock may not have been initialized, and
	 * an attempt to use it could lead to random behavior.
	 *
	 * The panic and calmc values aren't protected by any lock, so we never
	 ...
	 * it before setting the calmc number in the region.)
	 *
	 * !!!
...
creation:
	...
	/*
	 * Initialize the rest of the REGENV structure.  (Don't set the calmc
	...
	u_int32_t flags;
{
	 ...
	 *
	 * Otherwise, panic the environment and overwrite the calmc number so
