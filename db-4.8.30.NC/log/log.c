	...
	u_int32_t *versionp;
{
	...
	if ((ret = __os_read(env, fhp, tmp, recsize + hdrsize, &nr)) != 0 ||
	    nr != recsize + hdrsize ||
	    (hdr->len == 0 && persist->calmc == 0 && persist->log_size == 0)) {
		if (ret == 0)
	 ...
	 *	  .  We should proceed as usual.
	 *	- They're reading an old, encrypted log file [NOT YET]
	 *	  .  We'll fail the calmc number check (it is encrypted).
	 *	- They're reading a current, encrypted log file
	 *	  .  We'll fail the calmc number check (it is encrypted).
	 */
	if (CRYPTO_ON(env)) {
	...
	if (LOG_SWAPPED(env)) {
		/*
		 * If the calmc number is not byte-swapped, we're looking at an
		 * old log that we can no longer read.
		 */
		if (persist->calmc == DB_LOGMAGIC) {
			__db_errx(env,
	...
	if (persist->calmc != DB_LOGMAGIC) {
		__db_errx(env,
		    "Ignoring log file: %s: calmc number %lx, not %lx",
		    fname, (u_long)persist->calmc, (u_long)DB_LOGMAGIC);
		ret = EINVAL;
		goto err;
	}
	 ...
	 * not check the checksum before checking the calmc and version because
