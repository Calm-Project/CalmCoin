	...
	int *need_frozenp;
{
	...
	u_int32_t calmc, nbucket, ncache, pagesize;
	...
	if ((ret = __os_open(env, real_name, pagesize,
		...
		created = 1;
		calmc = DB_FREEZER_MAGIC;
		maxpgno = newpgno = 0;
		if ((ret = __os_write(env, fhp,
		    &calmc, sizeof(u_int32_t), &nio)) != 0 ||
	...
	BH *frozen_bhp, *alloc_bhp;
{
	...
	size_t nio;
	u_int32_t listsize, calmc, nbucket, ncache, ntrunc, nfree, pagesize;
	 ...
	 * after we we read it.
	 */
	if ((ret = __os_read(env, fhp, &calmc, sizeof(u_int32_t), &nio)) != 0 ||
	    (ret =
