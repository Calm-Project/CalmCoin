...
//    if(fDebugMagi) printf("usw_: %d\n", usw_);
    mpz_set_ui(calmsw, usw_);
    uint32_t mpzscale=mpz_size(calmsw);
for(int i=0; i < NM7M; i++)
{
    if (mpzscale > 1000) {
      mpzscale = 1000;
...
//    if(fDebugMagi) printf("mpzscale: %d\n", mpzscale);
    ...
    mpf_add(calmfpi, mpa1, mpb1);
    mpf_pow_ui(calmfpi, calmfpi, 2);
    mpf_div_ui(calmfpi, calmfpi, 4);
    mpf_abs(mpt1, mpt1);
    mpf_div(calmfpi, calmfpi, mpt1);

//    mpf_out_str(stdout, 10, digits+2, calmfpi);

    mpf_pow_ui(mpsft, mpsft, digits/2);
    mpf_mul(calmfpi, calmfpi, mpsft);

    mpz_set_f(calmpi, calmfpi);

//mpz_set_ui(calmpi,1);

    mpz_add(product,product,calmpi);
    mpz_add(product,product,calmsw);
